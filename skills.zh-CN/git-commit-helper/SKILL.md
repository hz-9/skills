...
- <a id="是否对话-Diff-路径"></a>**是否对话 Diff 路径**：标记当前是否处于对话直接提供 diff 的模式。由步骤 0.2 在对话 diff 校验通过时设置为 true，用于步骤 6.2 判断是否需要跳过推送确认。
...
4. **生成 commit message** — 生成候选方案并确认最终输出；
   ...
   4.2 用户选择 — 判断候选数量：
       - 候选数量 >= 2 -> 通过 AskUserQuestion 提供选项，阻塞等待用户选择：
         - [动态选项，由 AI 根据候选方案生成] -> 进入 4.3；
         - 都不合适 -> 通过 AskUserQuestion 询问方向，基于反馈重新生成后返回 4.1；若累计 3 次选择“都不合适”（连续计数，中间选择其他候选则重置；因复核检查退回 4.1 重新生成的，计数不清零，继续累计），引导用户手动输入 commit message，终止流程；
       - 候选数量 = 1 -> 通过 AskUserQuestion 提供选项，阻塞等待用户选择：
         - 接受该方案 -> 进入 4.3；
         - 都不合适 -> 通过 AskUserQuestion 询问方向，基于反馈重新生成后返回 4.1（累计“都不合适”计数）；
   ...
   4.4 关联 Issue — 询问用户是否需要关联 Issue；
       - 通过 AskUserQuestion 提供选项，阻塞等待用户选择：
         ...
         - 否，不关联 -> 跳过；
   4.5 确认汇总 — 汇总用户已确认的选择（选定方案、破坏性变更标记、关联 Issue），进入步骤 5；
...
5. **复核检查** — 对照 [Review List](#review-list)，确认提交信息内容；
   ...
   5.4 输出最终信息摘要（含 [Subject](#Subject) 长度、[Type](#Type)、[Body](#Body) 行数、[Footer](#Footer) 等维度），告知复核通过，进入步骤 6；

6. **成果输出** — 输出最终 commit message 并确认推送；
   6.1 输出最终 commit message 及完整结构化日志（含前置信息、候选方案、确认环节、最终输出等维度的汇总表格）；
   6.2 判断是否需要推送到远端：
       - 若当前为对话 Diff 路径 -> 跳过推送确认，直接告知“commit message 已生成完毕”；
       - 否则 -> 通过 AskUserQuestion 提供选项，阻塞等待用户选择：
         - 是，推送到远端 -> 获取当前分支的上游远程（通过 `git rev-parse --abbrev-ref --symbolic-full-name @{upstream}`）：
           - 成功（存在上游） -> 执行 `git push`：
             - 推送成功 -> 告知用户推送完成；
             - 推送失败 -> 告知用户推送失败的具体原因，提示用户手动处理；
           - 失败（无 upstream） -> 通过 `git remote` 获取所有远程仓库列表：
             - 存在至少一个远程 -> 取第一个远程名称，执行 `git push -u <remote_name> HEAD`：
               - 推送成功 -> 告知用户推送完成；
               - 推送失败 -> 告知用户推送失败的具体原因，提示用户手动处理；
             - 不存在任何远程 -> 提示用户没有可用的远程仓库，引导用户手动配置；
         - 否，暂不推送 -> 告知生成完成；
