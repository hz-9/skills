...
0. **前置检查** — 确保 commit 环境已就绪；
    - 初始化全局变量 [是否都不合适重试超限](#都不合适重试超限)、[是否复核检查重试超限](#复核检查重试超限) 均为 false；
    - 检测是否处于合并过程中（通过 `test -f "$(git rev-parse --git-dir)/MERGE_MSG"`）：
    ...
    - 判断 Git 版本是否 >= 2.0：
      - 是 -> 下一步；
      - 否 -> 提示升级 Git，终止流程；
    - 判断是否存在可供分析的 Git 变更（暂存区/工作区/commit/分支范围）：
      - 是 -> 下一步（进入步骤 1）；
      - 否 -> 告知用户无变更可分析，终止流程；
...
2. **获取 diff** — 根据输入来源获取变更内容；
   2.1 根据步骤 1 的判定结果映射到对应场景（"最近 commit"归入场景 B，commit 为 HEAD）：
       ...
       - 场景 B（单个 commit）：执行 `git rev-list --parents -n 1 <commit>` 检测根提交：
         ...
         - 进入 2.2；
       - 场景 C（分支范围）：执行 `git log <range> -p`，进入 2.2；
   2.2 完成获取后进入步骤 3；
