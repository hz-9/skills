...
3. **分析变更** — 分析 diff 内容并确定 commit 类型；
   ...
   3.2 输出 “分析变更内容”（按非二进制/二进制文件统计新增/修改/删除/重命名/权限变更数量），进入步骤 4；

4. **生成 commit message** — 生成候选方案并确认最终输出（4.3、4.4 请一起提问）；
   4.1 生成候选方案 — 基于分析结果选取 1~3 种合理 type，每种生成一个候选 commit message：
       - 若变更涉及多种性质（如新功能+重构）-> 考虑多个合理 type；
       - 若变更性质明确单一 -> 只生成 1 个候选；
       - 每个候选严格遵循 [Review List](#review-list) 的要求；
       - 输出 **提交信息多方案输出**
   ...
   4.4 关联 Issue — 询问用户是否需要关联 Issue；
       - 通过 AskUserQuestion 提供选项，阻塞等待用户选择：
         - 是，关联 Issue -> 由用户输入 Issue 编号（如 `#123`），补充到 footer，执行后进入 4.4；
         - 否，不关联 -> 进入 4.4；
   4.5 输出用户已确认的选择（选定方案、破坏性变更标记、关联 Issue），无需用户再次确认，进入步骤 5；
