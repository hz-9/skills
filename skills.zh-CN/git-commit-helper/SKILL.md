...
- <a id="Diff-格式校验重试超限"></a>**是否 Diff 格式校验重试超限**：标记用户在步骤 0.2 中连续提供格式不可解析的 diff 的次数是否达到 3 次上限。
...
0. **前置检查** — 确保 commit 环境已就绪；
  0.1 初始化全局变量 [是否复核检查重试超限](#复核检查重试超限)、[是否对话 Diff 路径](#是否对话-Diff-路径)、[是否 Diff 格式校验重试超限](#Diff-格式校验重试超限) 均为 false；
