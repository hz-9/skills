...
4. **执行决策** — 根据步骤 3 的用户选择执行操作；
   ...
   4.2 提交：
       - **⚠️ 必须使用 `NO_VERIFY=1` 环境变量跳过 git hooks，禁止不带此变量直接执行 `git commit`，以防 pre-commit 或 prepare-commit-msg hooks 阻塞提交**
...
- **提交规范**：提交时必须使用 `NO_VERIFY=1` 环境变量，禁止使用裸 `git commit` 调用，确保跳过 pre-commit、commit-msg 等 git hooks 检查；
