...
1. **分析** — 读取 `rush.json`，梳理项目数量、`workspace:*` 依赖关系、自定义 Rush 命令，向用户展示分析结果；
2. **初始化** — 在新仓库中创建 `pnpm-workspace.yaml`、`nx.json`、`.changeset/config.json`、根 `package.json`、`.npmrc`，详见 [迁移步骤](references/migration-steps.md#第-2-步-初始化-nx-workspace)；
3. **复制源码** — rsync 排除 node_modules/Rush 缓存，重组 `category/package` 目录为 `packages/`，详见 [迁移步骤](references/migration-steps.md#第-3-步-复制源码包)；
4. **创建 project.json** — 每个包创建 Nx project.json，配置 build/lint 命令和隐式依赖，详见 [迁移步骤](references/migration-steps.md#第-4-步-为每个包创建-projectjson)；
5. **迁移 Git hooks** — 创建 `.husky/commit-msg`、`.husky/pre-commit`、`commitlint.config.js`、`.lintstagedrc.json`，详见 [迁移步骤](references/migration-steps.md#第-5-步-迁移-git-hooks)；
6. **更新配置** — 更新 `.gitignore`、`.prettierrc.js`、`.prettierignore`、各包 `repository.directory`、`.vscode/settings.json`，详见 [迁移步骤](references/migration-steps.md#第-6-步-更新配置文件)；
7. **更新 CI/CD** — `actions-rush` → `pnpm/action-setup`，详见 [迁移步骤](references/migration-steps.md#第-7-步-更新-cicd)；
8. **验证** — `pnpm install && pnpm nx run-many --target=build --all`，确认 hooks 激活、构建通过，详见 [迁移步骤](references/migration-steps.md#第-8-步-安装依赖并验证)。
...
| 内部依赖 | 1 个（workspace:* → 转换为 standard version） |
...
- [ ] Rush 运行时文件（`rush.json`、`common/`、`.rush/`、`*.lint.log`）已清理
