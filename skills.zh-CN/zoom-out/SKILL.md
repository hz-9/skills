...
当 agent 对某段代码不熟悉时，触发 zoom-out 视角：拉远视角向上抽象一层，生成相关模块与调用者的全局地图，使用项目领域术语表词汇，帮助理解代码在整体架构中的位置。

## Definitions

- **Zoom Out**：从当前代码片段拉远视角向上抽象，展示模块间关系、调用链和架构角色的全景视角。
...
4. **复核检查** — 对照 [Review List](#review-list)，确认输出内容；
    ...
    - 依次判断 [Review List](#review-list) 中每个检查项，是否通过：
        - 基于“复核检查示例”来显示输出内容；
        - 有任一检查失败 -> 重新生成模块地图，返回步骤 3；
        - 全部通过 -> 下一步；

5. **成果输出** — 向用户呈现模块地图，说明目标代码如何融入全局；
