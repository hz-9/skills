...
NestJS 有三种 provider 作用域：DEFAULT（单例）、REQUEST（每个请求一个实例）和 TRANSIENT（每次注入一个新实例）。大多数 provider 应该是单例的。请求作用域的 provider 有性能影响，因为它们会通过依赖树向上冒泡。理解作用域可以防止内存泄漏和不正确的数据共享。
