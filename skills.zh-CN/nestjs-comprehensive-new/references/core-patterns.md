...
- 将仓储/ORM 代码放在使用领域语言的提供者后面，保持业务逻辑与数据访问分离
- 将事务性工作流隔离在拥有工作单元的服务中，而不是分散在多个控制器或服务中
- 不要让控制器直接协调多步写入操作
- 对于跨实体的多步操作，优先使用 Prisma 的交互式事务确保原子性
...
- 使用 `app.enableShutdownHooks()` 确保优雅关闭，在 `onModuleDestroy` 中清理资源
