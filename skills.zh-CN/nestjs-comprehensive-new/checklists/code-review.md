...
- [ ] PrismaService 使用 `$transaction` 进行事务操作，确保原子性
- [ ] 使用 `mockDeep<PrismaClient>()` 模拟 Prisma Service
- [ ] 单元测试中不包含实际的数据库依赖
...
- [ ] `prisma/schema.prisma` 中的模型定义正确，字段类型和关系完整
- [ ] PrismaService 实现了 `OnModuleInit` 和 `OnModuleDestroy` 管理连接生命周期
- [ ] 数据库连接字符串通过环境变量 `DATABASE_URL` 配置，不硬编码
- [ ] 查询使用 `select`/`include` 避免 N+1 问题
- [ ] 多步操作使用 `prisma.$transaction` 的交互式事务确保原子性
- [ ] Schema 变更后使用 `prisma migrate dev` 生成迁移
- [ ] 生产环境使用 `prisma migrate deploy` 部署迁移
- [ ] 每次 Schema 变更后运行 `prisma generate` 更新客户端
- [ ] 多步骤操作使用了事务
